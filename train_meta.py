...
def train(epoch):
    ...
    for batch_idx, (data, target) in enumerate(tqdm(train_loader)):
        ...
        if batch_idx % 301 == 300:
            ...
            logging('save weights to %s/%06d_%06d.weights' % (backupdir, epoch + 1, batch_idx))
            cur_model.seen = (epoch + 1) * len(train_loader.dataset)
            cur_model.save_weights('%s/%06d_%06d.weights' % (backupdir, epoch + 1, batch_idx))
