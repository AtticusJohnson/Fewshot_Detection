...
def load_conv_bn(buf, start, conv_model, bn_model):
    ...
    conv_model.weight.data.copy_(torch.from_numpy(buf[start:start+num_w]).reshape(conv_model.weight.data.shape)); start = start + num_w
    return start
...
if __name__ == '__main__':
    import sys
    blocks = parse_cfg('cfgs/yolo.cfgs')
    if len(sys.argv) == 2:
